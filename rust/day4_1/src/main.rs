...
fn main() {
    ...
    let solution = get_solution(draws, bingo_boards).unwrap();
...
fn get_solution(draws: Vec<u32>, mut bingo_boards: Vec<BingoBoard>) -> Option<u32> {
    for draw in draws {
...
2  0 12  3  7",
        ...
        let x = get_solution(draws, bingo_boards).unwrap();
        assert_eq!(x, 4512)
    }
}
